...
Such shifts in the development process might seems daunting at first, but let me show you how you could organize your code with those tools, and what will it brings to you and your users.
...
If your project is still structured around the MDAPI format, switching to the source format might save you from numerous bad merge and headaches, and here's some resources to help you do it:
...
A 'monolithic package' is a single Salesforce package containing the integrality of your product. A monolithic directory is constitued of a single package directory (a directory containing for example the lwc or object folder).

It contains all the files related to your package in a single folder.

For example, here's what would looks like a simple project with a simple console application with a layout and some Lightning Web Components:
...
### Con
- It's not scalable if you start getting a lot of feature: If you have two distinct feature sets, you might want to split your files between those two, but you won't be able to do so, because the subfolder of **package directory** are expecting files matching their names (so, LWC bundles in the `lwc` folder, Aura bundles in `aura` folder, but no LWC in a subfolder like `lwc/subfolder/myComponent`).
...
If they have some shared code/logic, it could be split in an 'util' directory. Also, you can can make multiple util directories. However, you should also apply the Separation of Concerns principle to thoses too.
...
```
Even with the files distributed among different folder, the Salesforce-CLI stitch them together as one in the end. Thanks to that, when using `sfdx force:source:push` or publishing the package, the result will still be the same as in Phase 1.
...
### Con
- It's quite different from the MDAPI file structure, so you might get confused at first if you're used to the the latter.
...
If not, this phase would mostly be the migration from the First Generation Packaging to the Second. However, not much information have been shared on the topic yet apart for an upcoming [developer preview](https://releasenotes.docs.salesforce.com/en-us/summer20/release-notes/rn_sfdx_packaging_preview_package_migration.htm)
...
> the `.placeholder` file is just there to ensure Git does commit the folder, you must also add it to your `.forceignore` file so that Salesforce CLI ignore it when pushing code.
...
## TL;DR and tips:
 ...
 - Use it smartly. Future you will thanks you for splitting and structuring your code today.
 - Baby steps, do it progressively. The phases are more like milestone, meaning that, at least for the file structure,  you can be in-between two phases, with an 'historic' folder for what existed before you decided to use the feature-oriented file structure. And then, when developing new features, refactor their dependencies.