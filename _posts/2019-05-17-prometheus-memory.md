...
tags: [Prometheus,Tsdb, Memory, Coveo]
...
Our Prometheus server was using a lot of memory for an amount of metrics which wasn't so big. So we decided to dive into tsdb.
...
But first let's take a quick overview of prometheus2 and its storage
...
This is 3 different time series from the 'up' metric.

**Target** : Monitoring endpoint that expose metrics in the prometheus format

**Chunk** : Batch of scraped time series.

**Series churn** : describe that a set of time series becomes inactive, i.e. receives no more data points, and a new set of active series appears instead. Rolling update can create this kind of situation.
...
**Head Block** : The currently open block where all incoming chunk are written.

**Sample** : A collection of all datapoint grap on a target in one scrap.
...
So we decided to copy the ebs storing our data from prometheus and mount it on a dedicated instance to run the analyze.
```
...
Regarding to this statistics we can see that the monitoring of the kubelet service seems to generate a lot of churn (which is normal considering that it expose all container metrics and that container rotate often ) and that the id label has an high cardinality.

## Actions

Here the only action we will take will be to drop the id label which brings any interesting information.
